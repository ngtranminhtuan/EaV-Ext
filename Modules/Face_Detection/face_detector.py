...
class FaceDetector(object):
    ...
    def loadColors(self):
        num_classes = len(FaceDetector.label_names)
        hsv_tuples = [(1.0 * x / num_classes, 1.0, 1.0) for x in range(num_classes)]
        colors = list(map(lambda x: colorsys.hsv_to_rgb(*x), hsv_tuples))
        FaceDetector.colors = list(
            map(lambda x: (int(x[0] * 255), int(x[1] * 255), int(x[2] * 255)), colors)
        )

    def loadLabels(self, labelsPath):
        with open(labelsPath, "r") as f:
            FaceDetector.label_names = [line.strip() for line in f.readlines()]
    ...
    def postprocess(self, raw_predictions, org_image, score_thresh, nms_thresh):
        ...
        if FaceDetector.orgW and FaceDetector.orgH:
            h_org_image, w_org_image = FaceDetector.orgH, FaceDetector.orgW
        else:
    ...
    @staticmethod
    def drawBoxesTracking(img, postprocessedData):
        filteredBoxes = []
        for result in postprocessedData:
            ...
            ymax = result[3]
            color = (255,0,0)
            img = rectangle_with_text(
                img, class_name.capitalize(), (xmin, ymin), (xmax, ymax), color, 2
            )
            filteredBoxes.append((xmin, ymin, xmax, ymax))
        return img, filteredBoxes
    ...
    def drawBoxes(img, postprocessedData):
        ...
        for result in postprocessedData:
            ...
            xmax = result[4]
            ymax = result[5]
            color = FaceDetector.colors[FaceDetector.label_names.index(class_name)]
            img = rectangle_with_text(
                img, class_name.capitalize(), (xmin, ymin), (xmax, ymax), color, 2
            )
            filteredBoxes.append((xmin, ymin, xmax, ymax))
    ...
    def setOrgSize(imgW, imgH):
        FaceDetector.orgW = imgW
        FaceDetector.orgH = imgH
...
def runImg(logging, img, outputDir, objectDetector, scoreThresh, nmsThresh):
    ...
    FaceDetector.drawBoxes(frame, postprocessedData)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output", type=str, default="",   help="Path of output video file")
