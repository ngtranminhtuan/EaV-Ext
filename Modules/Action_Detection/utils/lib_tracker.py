...
class Tracker(object):
    ...
    def track(self, curr_skels):
        ...
        if len(self._dict_id2skeleton) > 0:
            ...
            for i2, i1 in good_matches.items():
                ...
                idx = orgSkeletons.index(curr_skels[i2])
                self._dict_id_idx[human_id] = idx
        ...
        for i in range(num_humans_to_add):
            ...
            idx = orgSkeletons.index(curr_skels[unmatched_idx[i]])
            self._dict_id_idx[self._cnt_humans] = idx
        
        return self._dict_id2skeleton, self._dict_id_idx
    ...
    def _sort_skeletons_by_dist_to_center(self, skeletons):
        ...
        def cmp(a, b): return np.subtract((a > b), (a < b), dtype=np.float32)
