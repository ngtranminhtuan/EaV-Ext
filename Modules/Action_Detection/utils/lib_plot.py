...
def draw_action_result(img_display, id, skeleton, str_action_label):
    ...
    (label_width, label_height), baseline = cv2.getTextSize(str_action_label, font, fontsize, cv2.LINE_AA)

    

    TEST_COL = int((minx+maxx)/2 - label_width * box_scale)
    TEST_ROW = int(miny - 3*label_height * box_scale)

    if str_action_label:
        # img_display = rectangle(img_display, (TEST_COL,TEST_ROW-label_height),(TEST_COL+label_width,TEST_ROW), (255,255,255), 2)
        img_display = rounded_rectangle(img_display, (TEST_COL-10, TEST_ROW-label_height-10),(TEST_ROW+10, TEST_COL+label_width+10,), color= (51, 51, 51), radius=0.7, thickness=-1)
        # res = cv2.addWeighted(sub_img, 0.5, white_rect, 0.5, 1.0)

    img_display = cv2.putText(
        img_display, str_action_label.capitalize(), (TEST_COL+7, TEST_ROW-5), font, fontsize, (0, 190, 246), linewidth, cv2.LINE_AA)
